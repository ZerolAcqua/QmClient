...
function CreateRealtimeServer(Server, { Recognition, DeveloperService, TitleService, NewsService, Playtime, NowSec = () => Math.floor(Date.now() / 1000) })
{
	...
	function Users(Socket, Data)
	{
		if(!Data) return;
		const Session = Sessions.get(Socket);
		if(!Session) return;
		const Users = Data.users.map(({ last_ip, ...User }) => User);
		Send(Socket, "users", { users: Users, server_address: Session.Presence.server_address, lease_seconds: 20 });
	}
	...
	function Report(Socket, Session, Body)
	{
		...
		if(PreviousAddress !== Presence.server_address) Users(Socket, Recognition.Current());
	}
		...
		Socket.on("message", (Raw, Binary) => {
			...
			try
			{
				...
				if(Body.type === "hello")
				{
					...
					Report(Socket, Session, Body);
					Profile(Socket, Session);
					Scoped(Socket, Session);
					Users(Socket, Recognition.Current());
	...
	const LeaseTimer = setInterval(() => {
		for(const [Socket, Session] of Sessions)
		{
			try { Scoped(Socket, Session); Time(Socket, Session); }
			catch { Error(Socket, "service_unavailable"); }
		}
	}, 5000);
	LeaseTimer.unref();
