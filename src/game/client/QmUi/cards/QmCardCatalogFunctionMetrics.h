...
namespace qm_card_catalog
{
	...
	inline const std::array<SQmMiniFeatureRow, 16> &QmMiniFeatureRows()
	{
		// 行文案以 Localizable 标注：表在卡片目录里，渲染侧只做 Localize(m_pTextId)，
		// 不加标注翻译提取脚本就看不到这些 source key，语言文件会整行退回英文。
		static const std::array<SQmMiniFeatureRow, 16> s_aRows = {{
			...
			{&g_Config.m_QmProcessHighPriority, Localizable("High process priority"), &g_Config.m_QmProcessHighPriority},
