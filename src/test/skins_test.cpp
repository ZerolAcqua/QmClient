...
TEST(Skins, TeeSkinUploadConsumesOneSourceTextureBudgetBeforeUpload)
{
	std::ifstream File(TestSourcePath("src/game/client/components/skins.cpp"));
	ASSERT_TRUE(File.good());
	std::stringstream Buffer;
	...
	EXPECT_NE(ProcessSkinBody.find("CanUpload(1)"), std::string::npos);
	EXPECT_EQ(ProcessSkinBody.find("CanUpload(SETTINGS_SKIN_SOURCE_TEXTURE_UPLOADS)"), std::string::npos);
